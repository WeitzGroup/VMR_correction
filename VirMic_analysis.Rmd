...
data$DeepShallowColor <- factor(data$DEPTH > 100, labels=c("chartreuse3", "blue"))
...
map <- worldmap + 
  ...
  guides(colour = guide_legend(override.aes = list(shape=c(20,15),size=c(7,7),color=c("blue","chartreuse3"),alpha=c(1,1))))
map
...
jpeg("output/figures/jpg/map.jpg", width=1500, height=900)
print(map)
dev.off()

png("output/figures/jpg/map.png", width=1500, height=900)
print(map)
dev.off()

tiff("output/figures/jpg/map.tiff", width = 10, height = 6, units = 'in', res = 300)
print(map)
dev.off()
...
small$DepthCategory[small$DEPTH >  100] <- ">100m"
small$color<-ifelse(small$DepthCategory == "<=100m","chartreuse3","blue")
...
small$DepthCategory<-factor(small$DepthCategory)

require(grid)
logRatioDistribution<-ggplot(small, aes(x=Log10_VirBac_Ratio, y=..count..,fill=DepthCategory)) +
  geom_histogram(binwidth=.1,show.legend = F) +
  facet_grid(DepthCategory ~ .) +
  scale_fill_manual(values = c("<=100m" = "blue", ">100m" = "chartreuse3")) +
  geom_segment(data=ann,aes(x=med,xend=med,y=medValuePlusTwenty,yend=medValue),arrow=arrow(length=unit(0.5,"cm")), color="#D55E00") +
  geom_segment(data=ann,aes(x=iqr25,xend=iqr25,y=-20,yend=0),arrow=arrow(length=unit(0.5,"cm")),show.legend = F, color="#0072B2") +
  geom_segment(data=ann,aes(x=iqr975,xend=iqr975,y=-20,yend=0),arrow=arrow(length=unit(0.5,"cm")),show.legend=F, color="#0072B2") +
  geom_text(data=ann,aes(x=med,y=medValuePlusThirty,label=medtext),size=5,show.legend=F) +
  geom_text(data=ann,aes(x=iqr25,y=-30,label=text25),size=5,show.legend=F) +
  geom_text(data=ann,aes(x=iqr975,y=-30,label=text975),size=5,show.legend=F) +
...
RatioDistribution<-ggplot(small, aes(x=VIRUS/BACTERIA, y=..count.., fill=DepthCategory)) +
...
ms <- summary(m1)
RSQ10BacModel<-list(r2 = format(1-(sum((smallLabSet$log10VIRUS-(1+(1*smallLabSet$log10BACTERIA)))^2)/                                     sum((smallLabSet$log10VIRUS-mean(smallLabSet$log10VIRUS))^2)),digits = 3))
...
WIDTH<-7.5
HEIGHT<-9.5
vp1 <- viewport(width = 4.1/10, height = 4.1/12, x = 2/10, y = 10.1/12)
vp2 <- viewport(width = 3/10, height = 2.4 /12, x = 5/10, y = 10.8/12)
vp3 <- viewport(width = 3/10, height = 2.4/12, x = 7/10, y = 10.8/12)
vp4 <- viewport(width = 3/10, height = 2.4/12, x = 9/10, y = 10.8/12)
...
print(graphSmall(FECYCLE1), vp = vp6)
print(graphSmall(FECYCLE2), vp = vp7)
print(graphSmall1(GEOTRACES), vp = vp8)
print(graphSmall(GEOTRACES_LEG3), vp = vp9)
print(graphSmall(GREENLAND2012), vp = vp10)
print(graphSmall(INDIANOCEAN2006), vp = vp11)
print(graphSmall(KH04_5), vp = vp12)
print(graphSmall1(KH05_2), vp = vp13)
print(graphSmall(MOVE), vp = vp14)
print(graphSmall(NASB2005), vp = vp15)
print(graphSmall(NORTHSEA2001), vp = vp16)
print(graphSmall(POWOW), vp = vp17)
print(graphSmall1(RAUNEFJORD2000), vp = vp18)
print(graphSmall2(SOG), vp = vp19)
print(graphSmall2(STRATAPHYT1), vp = vp20)
print(graphSmall2(STRATAPHYT2), vp = vp21)
print(graphSmall2(TABASCO), vp = vp22)
print(graphSmall3(USCMO), vp = vp23)
dev.off()
...
smallLabSet1$Study <- factor(smallLabSet1$Study, levels = c("POWOW","TABASCO","FECYCLE2","FECYCLE1","NASB2005","INDIANOCEAN2006","STRATIPHYT2","SOG","GEOTRACES_LEG3","GREENLAND2012","MOVE","STRATIPHYT1","RAUNEFJORD2000","KH05_2","GEOTRACES","KH04_5","NORTHSEA2001","USC MO","BEDFORDBASIN","CASES03-04","ARCTICSBI","BATS"))

x1<-x
x1$Study <- factor(x$Study, levels = c("POWOW","TABASCO","FECYCLE2","FECYCLE1","NASB2005","INDIANOCEAN2006","STRATIPHYT2","SOG","GEOTRACES_LEG3","GREENLAND2012","MOVE","STRATIPHYT1","RAUNEFJORD2000","KH05_2","GEOTRACES","KH04_5","NORTHSEA2001","USC MO","BEDFORDBASIN","CASES03-04","ARCTICSBI","BATS"))
...
p<-ggplot(x1, aes(x=Study, y=slopes)) +
  ...
  theme(title = element_text(size=20),
        ...
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        axis.text.y = element_text(colour = c("grey50", "grey50", "grey50", "grey50", "black", "grey50", "black",  "black",  "grey50",  "black", "black",  "black",  "black",  "black",  "black",  "black",  "black",  "black",  "black",  "black",  "black", "black",  "black"))) + 
...
names(smallLabSet1)
ag<-aggregate(x1$slopes,by=list(x1$Study),FUN = function(x) t.test(x)$conf.int[1:2])
ag$containsOneColor<-ifelse(ag$x[,1]<=1 & ag$x[,2]>=1,"grey50","black")
ag$Study <- ag$Group.1 
smallLabSet1<-merge(smallLabSet1,ag[,c(3,4)])
names(smallLabSet1)
hist_right <- ggplot()+
  #geom_histogram(aes(smallLabSet1$Study,fill=smallLabSet1$color)) +
  stat_count(aes(smallLabSet1$Study,fill=smallLabSet1$color),show.legend = F) +
...
hist_right <- ggplot()+
  #geom_histogram(aes(smallLabSet1$Study,fill=smallLabSet1$color)) +
  stat_count(aes(DeepSmall1$Study,fill=DeepSmall1$color),show.legend = F) +
  scale_fill_manual(name="One in CI?", values = c("black","blue"), labels=c('No','Yes')) +
  coord_flip() +
  labs(list(y='Frequency')) + 
  theme(title = element_text(size=20),
...
shallowconstrained<-ggplot(smallLabSet, aes(x=log10BACTERIA, y=log10VIRUS, colour=Study)) +
  geom_point(size=.01, alpha=.01) +
  geom_abline(intercept=3.9203, slope=0.5195, col='darkgreen', size=.25, alpha=1) +
  geom_abline(intercept=4.0692870, slope=0.5195, col='darkgreen', size=.25, alpha=1) +
  geom_abline(intercept=3.8319, slope=0.5195, col='darkgreen', size=.25, alpha=1) +
  geom_abline(intercept=4.0692870, slope=0.5195, col='darkgreen', size=.25, alpha=1) +
  geom_abline(intercept=3.8319, slope=0.5195, col='darkgreen', size=.25, alpha=1) +
  geom_abline(intercept=3.8319, slope=0.5195, col='darkgreen', size=.25, alpha=1) +
  geom_abline(intercept=3.8319, slope=0.5195, col='darkgreen', size=.25, alpha=1) +
  ...
  geom_abline(intercept=3.8319, slope=0.5195, col='darkgreen', size=.25, alpha=1) +
  geom_abline(intercept=3.8319, slope=0.5195, col='darkgreen', size=.25, alpha=1) +
  geom_abline(intercept=4.0040, slope=0.5195, col='darkgreen', size=.25, alpha=1) +
  geom_abline(intercept=4.0692870, slope=0.5195, col='darkgreen', size=.25, alpha=1) +
  geom_abline(intercept=3.8319, slope=0.5195, col='darkgreen', size=.25, alpha=1) +
  geom_abline(intercept=3.8319, slope=0.5195, col='darkgreen', size=.25, alpha=1) +
  geom_abline(intercept=3.8319, slope=0.5195, col='darkgreen', size=.25, alpha=1) +
...
ann4 <- data.frame(
  ...
  DeepShallow = c('<=100m', '>100m'),
  constrainedslopes = c(0.5125,0.5325704),
  constrainedints1 = c(3.9203,3.4757200),
  constrainedints2 = c(4.0693,3.5510580),
  constrainedints3 = c(3.8319,3.5125687),
  constrainedints4 = c(4.0040,3.5190784),
  constrainedints5 = c(3.8951,3.5190784),
  constrainedints6 = c(4.0693,3.5190784),
  constrainedints7 = c(3.8319,3.5190784),
  constrainedints8 = c(4.0693,3.5190784)
  )
...
s$Study <- factor(s$Study, levels = c("slope", "ARCTICSBI","BATS","BEDFORDBASIN","CASES0304","FECYCLE1","FECYCLE2","GEOTRACES","GEOTRACES_LEG3","GREENLAND2012","INDIANOCEAN2006","KHO4","KH05_2","MOVE","NASB2005","NORTHSEA2001","POWOW","RAUNEFJORD","SOG","STRATAPHYT1","STRATAPHYT2","TABASCO","USCMO"))
s<-s[order(s$Study),]
rownames(s) <- NULL
#kable(s)
m1<-lm(log10VIRUS~log10BACTERIA,data=smallLabSet)
...
summary(shallowslopes)
quantile(shallowslopes, c(.025,.975))
summary(deepslopes)
quantile(deepslopes, c(.025,.975))
