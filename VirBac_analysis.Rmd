...
```{r, echo = FALSE, warning=F, results='hold', message=FALSE}
setwd("C:\\Users\\cwigington3\\Dropbox (Weitz Group)\\NatureMicro_noELATROUTSWAT\\Code_and_Figures")
load("DATA.RData")
head(data)
world <- map_data("world")
index <- which(world$long > 178 & world$long < 190.3)
clean.world <- world[-index, ]
data<-data[-which(data$Study %in% c("ELA","TROUT","SWAT")),]
...
Allscatterplot<- ggplot(small, aes(x=log10BACTERIA, y=log10VIRUS, color=DepthCategory)) +
  ...
  #geom_density_2d(aes(colour = DepthCategory)) +
...
Histograms of the ratio of viral abundance to Microbe abundance and the log of the ratio of viral abundance to Microbe abundance. The histograms of ratio of viral abundance to Microbe abundance is shown for depth categories. The distribution at the top of the panel represents samples taken at depths of 100 meters or less; samples taken at depths of greater than 100 meters are shown in the bottom panel. The red arrow at the top shows the position of the median value. The blue arrows shows the range of value between which 95 percent of the data is found. The domain of ratio values shown include 99\% of all of the ratio data points.
...
small$label<-ifelse(small$DepthCategory == ">100m","Above 100 m","Below or at 100 m")
ann$label<-ifelse(ann$DepthCategory == ">100m","Above 100 m","Below or at 100 m")
require(grid)
logRatioDistribution <- ggplot(small, aes(x=Log10_VirBac_Ratio, y=..count.., fill=factor(col))) +
  geom_histogram(binwidth=.1) +
  facet_grid(label ~ .) +
  #scale_fill_manual(values = c("chartreuse3", "blue")) +
  ...
  labs(list(x=expression('Ratio of viral abundance to microbe abundance (on log'[10]*' scale)'), 
            y='Frequency'))+ 
  theme(plot.title=element_text(size=32),
        aspect.ratio=1,
        axis.text=element_text(size=16),
...
# small$label<-ifelse(small$DepthCategory == ">100m","Above or at 100 m","Below 100 m")
# small$median_arrow_text <- ifelse(small$DepthCategory == ">100m","Hello","Goodbye")
# par(mfrow=c(2,1),mar=c(3,4,1,1))
# lapply(split(small,list(small$DepthCategory)), function(d)hist(d$Log10_VirBac_Ratio,main=paste(unique(d$label),sep="."), col=d$col, xlim=c(-1,3.0), ylim=c(0,400), breaks = c(seq(-1,3,length=50))))
# lapply(split(small,list(small$DepthCategory)), function(d)text(1, 1.2, unique(d$median_arrow_text)))
# 
# require(graphics)

split.screen(rbind(c(0.05,0.95,0.1, 0.55), c(0.05, 0.95, 0.55, 1)))
screen(2)
par(mgp=c(1.2,.4,0), mar = c(rep(2,4))) 
hist(small$Log10_VirBac_Ratio[which(small$label == 'Below or at 100 m')], main = 'Below or at 100 m', col='blue', xlim=c(-1,3.0), ylim=c(0,400), breaks = c(seq(-1,3,length=50)), xaxt='n', xlab='', xaxs="i", yaxs="i")
text(ann$med[which(ann$label == 'Below or at 100 m')], ann$medValue[which(ann$label == 'Below or at 100 m')], ann$medtext[which(ann$label == 'Below or at 100 m')], cex = 1.2)
text(ann$iqr25[which(ann$label == 'Below or at 100 m')], 75, ann$text25[which(ann$label == 'Below or at 100 m')], cex = 1.2)
text(ann$iqr975[which(ann$label == 'Below or at 100 m')], 85, ann$text975[which(ann$label == 'Below or at 100 m')], cex = 1.2)
arrows(ann$med[which(ann$label == 'Below or at 100 m')], 318, ann$med[which(ann$label == 'Below or at 100 m')], 278, col = 2,lwd=2)
arrows(ann$iqr25[which(ann$label == 'Below or at 100 m')], 60, ann$iqr25[which(ann$label == 'Below or at 100 m')], 30, col = 4,lwd=2)
arrows(ann$iqr975[which(ann$label == 'Below or at 100 m')], 70, ann$iqr975[which(ann$label == 'Below or at 100 m')], 40, col = 4,lwd=2)

screen(1)
par(mgp=c(1.2,.4,0), mar = c(rep(2,4))) 
hist(small$Log10_VirBac_Ratio[which(small$label == 'Above 100 m')], main = 'Above 100 m', col='chartreuse3', xlim=c(-1,3.0), ylim=c(0,400), breaks = c(seq(-1,3,length=50)), xlab = 'Ratio of viral abundance to microbial abundance (on log10 scale)')
text(ann$med[which(ann$label == 'Above 100 m')], ann$medValue[which(ann$label == 'Above 100 m')], ann$medtext[which(ann$label == 'Above 100 m')], cex = 1.2)
text(ann$iqr25[which(ann$label == 'Above 100 m')], 85, ann$text25[which(ann$label == 'Above 100 m')], cex = 1.2)
text(ann$iqr975[which(ann$label == 'Above 100 m')], 85, ann$text975[which(ann$label == 'Above 100 m')], cex = 1.2)
arrows(ann$med[which(ann$label == 'Above 100 m')], 338, ann$med[which(ann$label == 'Above 100 m')], 298, col = 2,lwd=2)
arrows(ann$iqr25[which(ann$label == 'Above 100 m')], 60, ann$iqr25[which(ann$label == 'Above 100 m')], 30, col = 4,lwd=2)
arrows(ann$iqr975[which(ann$label == 'Above 100 m')], 70, ann$iqr975[which(ann$label == 'Above 100 m')], 40, col = 4,lwd=2)
#axis(1, at = seq(0, 30, 20))
close.screen(all.screens = TRUE)


pdf("output/figures/pdf/logRatioDistribution_base.pdf", width=4, height=7)
split.screen(rbind(c(0,1,0, 0.52), c(0, 1, 0.48, 1)))
screen(2)
par(mgp=c(2,.6,0), #the first argument changes the distance between the label and the number, the second argument changes the distance between the numbers and the dishes
    mar = c(4,3,1,1)) #margin widths, first is bottom, second argument is left
hist(small$Log10_VirBac_Ratio[which(small$label == 'Below or at 100 m')], main = 'Near-surface (<=100 m)', col='blue', xlim=c(-1,3.0), ylim=c(0,400), breaks = c(seq(-1,3,length=50)), xaxt='n', xlab='', xaxs="i", yaxs="i", font.lab=1)
axis(2, font=1)
text(ann$med[which(ann$label == 'Below or at 100 m')], ann$medValue[which(ann$label == 'Below or at 100 m')], ann$medtext[which(ann$label == 'Below or at 100 m')], cex = 1.2)
text(ann$iqr25[which(ann$label == 'Below or at 100 m')], 75, ann$text25[which(ann$label == 'Below or at 100 m')], cex = 1.2)
text(ann$iqr975[which(ann$label == 'Below or at 100 m')], 85, ann$text975[which(ann$label == 'Below or at 100 m')], cex = 1.2)
...
screen(1)
par(mgp=c(2,.6,0), mar = c(4,3,2,2)) 
hist(small$Log10_VirBac_Ratio[which(small$label == 'Above 100 m')], main = 'Sub-surface (>100 m)', col='chartreuse3', xlim=c(-1,3.0), ylim=c(0,400), breaks = c(seq(-1,3,length=50)), xlab = 'Viral-microbial abundance ratio (on log10 scale)', font.lab=1 )
axis(1, font=1)
axis(2, font=1)
text(ann$med[which(ann$label == 'Above 100 m')], ann$medValue[which(ann$label == 'Above 100 m')], ann$medtext[which(ann$label == 'Above 100 m')], cex = 1.2)
text(ann$iqr25[which(ann$label == 'Above 100 m')]-.1, 85, ann$text25[which(ann$label == 'Above 100 m')], cex = 1.2)
text(ann$iqr975[which(ann$label == 'Above 100 m')]+.13, 85, ann$text975[which(ann$label == 'Above 100 m')], cex = 1.2)
arrows(ann$med[which(ann$label == 'Above 100 m')], 338, ann$med[which(ann$label == 'Above 100 m')], 298, col = 2,lwd=2)
arrows(ann$iqr25[which(ann$label == 'Above 100 m')], 60, ann$iqr25[which(ann$label == 'Above 100 m')], 30, col = 4,lwd=2)
arrows(ann$iqr975[which(ann$label == 'Above 100 m')], 70, ann$iqr975[which(ann$label == 'Above 100 m')], 40, col = 4,lwd=2)
#axis(1, at = seq(0, 30, 20))
...
jpeg("output/figures/jpg/logRatioDistribution_base.jpg", width=400, height=500)
split.screen(rbind(c(0.05,0.95,0.01, 0.50), c(0.05, 0.95, 0.51, 1)))
screen(2)
par(mgp=c(1.2,.6,0), mar = c(rep(2,4))) 
hist(small$Log10_VirBac_Ratio[which(small$label == 'Below or at 100 m')], main = 'Below or at 100 m', col='blue', xlim=c(-1,3.0), ylim=c(0,400), breaks = c(seq(-1,3,length=50)), xaxt='n', xlab='', xaxs="i", yaxs="i")
text(ann$med[which(ann$label == 'Below or at 100 m')], ann$medValue[which(ann$label == 'Below or at 100 m')], ann$medtext[which(ann$label == 'Below or at 100 m')], cex = 1.2)
text(ann$iqr25[which(ann$label == 'Below or at 100 m')], 75, ann$text25[which(ann$label == 'Below or at 100 m')], cex = 1.2)
text(ann$iqr975[which(ann$label == 'Below or at 100 m')], 85, ann$text975[which(ann$label == 'Below or at 100 m')], cex = 1.2)
...
screen(1)
par(mgp=c(1.2,.4,0), mar = c(2.2,2,2,2)) 
hist(small$Log10_VirBac_Ratio[which(small$label == 'Above 100 m')], main = 'Above 100 m', col='chartreuse3', xlim=c(-1,3.0), ylim=c(0,400), breaks = c(seq(-1,3,length=50)), xlab = 'Ratio of viral abundance to microbial abundance (on log10 scale)', font.lab=2 )
axis(1, lwd = 1, font.lab=2 )
text(ann$med[which(ann$label == 'Above 100 m')], ann$medValue[which(ann$label == 'Above 100 m')], ann$medtext[which(ann$label == 'Above 100 m')], cex = 1.2)
text(ann$iqr25[which(ann$label == 'Above 100 m')], 85, ann$text25[which(ann$label == 'Above 100 m')], cex = 1.2)
text(ann$iqr975[which(ann$label == 'Above 100 m')], 85, ann$text975[which(ann$label == 'Above 100 m')], cex = 1.2)
arrows(ann$med[which(ann$label == 'Above 100 m')], 338, ann$med[which(ann$label == 'Above 100 m')], 298, col = 2,lwd=2)
...
#TenToOnersq<- 1-(sum((smallLabSet$log10VIRUS-(1+(smallLabSet$log10BACTERIA)))^2)/sum((smallLabSet$log10VIRUS-mean(smallLabSet$log10VIRUS))^2))

d=data.frame(x=c(9.5,10), y=c(TenToOnersq,TenToOnersq+.1), vx=c(10.5,10), vy=c(TenToOnersq,TenToOnersq-.1))
...
shallowlinearmods<-ggplot(data=df, aes(x=10^ints, y=rsqs)) +
  ...
  geom_segment(data=d, mapping=aes(x=x, y=y, xend=vx, yend=vy), size=1.3, color="#ff8000") +
